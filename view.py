...
def MENU():
    ...
    while True:       
        ...
        if i == 6:
            cc = input("Choose which filtrarion u wanna use\n 1 - find_all_students_on this faculty\n 2 - find_all_students_whoose_group_count_is_more_then\n")
            cc = int(cc)
            os.system('cls||clear')
            if cc == 1:
            ...
            if cc == 3:
                ...
                for row in records:  
                    print("---------------")              
                    print("gr name = ", row[0], )
                    print("count of students in group = ", row[1])
                    print("dean  = ", row[2])
    cursor.close()
    connection.close()
