...
def who_is_a_dean_4_yhis_group(name):
    try:      
        ...
        connection = psycopg2.connect(host="localhost", port = 5432, database="Lab1", user="postgres", password="1234")
        cursor = connection.cursor()
        query = """with x as(SELECT fac_name  from public.group WHERE group_name = %s)
        select    dean from public.faculty h inner join(select* from x) m on h.fac_name = m.fac_name"""
        start = time.time()
